...
class accesos
{
		...
		switch($nombre_tab)
		{
			case "nav-tab-ajuste_generales":
				$tab_html = '	<a class="nav-item nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-home" role="tab" aria-controls="nav-home" aria-selected="true">Roles de Usuario</a>
								...
								<a class="nav-item nav-link" id="nav-permisos-tab" data-toggle="tab" href="#nav-permisos" role="tab" aria-controls="nav-permisos" aria-selected="false">Acceso a Modulos</a>
    ...
    public function creacion_tabla($nombre_tabla,$_tipo_tabla)
    {
        ...
        switch ($nombre_tabla) 
		{
			...
			case "tabla_periodo":
				$tabla_html = '	<table id="tabla_periodo" class="table table-striped table-bordered table-condensed w-100">
									<thead class="text-center">
										<tr>
											<th>ID</th>
											<th>AÑO</th>
        ...
        switch ($nombre_tabla) 
		{
			...
			case "tabla_permisos":
				...
				$columnas_html = '	[	{"data": "permiso_id"},
                    					{"data": "per_usuario_id"},
										{"data": "per_nombre_corto"},
                    					{"data": "per_modulo_nombre"},
			...
			case "form_nuevo_periodo":
				switch($nombre_objeto)
				{
					case "btn_nuevo_periodo":
						$botones_formulario = ' <button id="btn_nuevo_periodo" type="button" class="btn btn-secondary btn-sm btn_nuevo_periodo" data-toggle="modal">+ Nuevo</button> ';
					break;
				}
			break;
